...
public class SanPhamChiTietServiceImpl implements SanPhamChiTietService {
    ...
    @Override
    public java.io.ByteArrayInputStream exportToExcel(
            String search, Long idMauSac, Long idKichThuoc, 
            BigDecimal minPrice, BigDecimal maxPrice, 
            String trangThai, Long idSanPham, 
            Long idThuongHieu, Long idDanhMuc
        ...
        org.springframework.data.jpa.domain.Specification<SanPhamChiTiet> spec = (root, query, cb) -> {
            ...
            if (idSanPham != null && idSanPham > 0) predicates.add(cb.equal(root.get("sanPham").get("id"), idSanPham));
            if (idMauSac != null && idMauSac > 0) predicates.add(cb.equal(root.get("mauSac").get("id"), idMauSac));
            if (idKichThuoc != null && idKichThuoc > 0) predicates.add(cb.equal(root.get("kichThuoc").get("id"), idKichThuoc));
            ...
            if (idThuongHieu != null && idThuongHieu > 0) predicates.add(cb.equal(root.get("sanPham").get("thuongHieu").get("id"), idThuongHieu));
            if (idDanhMuc != null && idDanhMuc > 0) predicates.add(cb.equal(root.get("sanPham").get("danhMuc").get("id"), idDanhMuc));
